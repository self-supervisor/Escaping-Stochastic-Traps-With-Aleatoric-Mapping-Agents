...
def a2c_algo(request):
    ...
    for i in range(procs):
        an_env = utils.make_env(env, int(frames_before_reset), int(environment_seed))
        an_env = ResetWrapper(an_env, int(frames_before_reset), int(environment_seed))
        envs.append(an_env)
...
def test_add_noisy_tv(a2c_algo):
    ...
    obs_with_tv = a2c_algo.env.add_noisy_tv(obs, action)
    ...
    for an_env in a2c_algo.env.envs:
        an_env.algo_count = 8
        an_env.frames_before_reset = 8
        an_env.reset_environments_if_ness()
        grids.append(str(an_env)) 
        positions.append(an_env.agent_pos)    
        an_env.reset_count = 0
...
def test_reset_environments_if_ness(a2c_algo):
    ...
    for _ in range(10):
        ...
        assert all_same(grids)
        assert all_same(positions)
        #random_env_index = random.choice(list(range(16)))
        #a2c_algo.env.envs[3].frames_before_reset = 1e6
        ...
        #for an_env in a2c_algo.env.envs:
        #    reset_counts.append(an_env.reset_count)
        
        #assert all_same(reset_counts) == False
        
        #https://stackoverflow.com/questions/176918/finding-the-index-of-an-item-in-a-list 
        
        #num_of_zeros = [i for i,x in enumerate(reset_counts) if x == 0]
        #assert num_of_zeros == 1
        #import pdb; pdb.set_trace()
        # point is that above the random env shouldn't reset
